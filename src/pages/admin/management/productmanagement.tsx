...
const Productmanagement = () => {
 ...
 console.log("data from products:",data)
  const { price, ProductImage, ProductName, stock, category, description } =
    data?.product || {
      ProductImage: [],
      category: "",
      ProductName: "",
  ...
  const [nameUpdate, setNameUpdate] = useState<string>(ProductName);
  ...
  useEffect(() => {
    if (data) {
      setNameUpdate(data.product.ProductName);
  ...
  return (
    <div className="admin-container">
      <AdminSidebar />
      <main className="product-management manage-product">
        {isLoading ? (
          <Skeleton length={20} />
        ) : (
          <>
            <section>
              ...
              <img src={transformImage(ProductImage[0]?.url)} alt="Product" />
              <p>{ProductName}</p>
              {stock > 0 ? (
